...
for (var i = 0; i < numImgs; i++) {
	var count = 0;
	catImages.push(fetch('http://thecatapi.com/api/images/get?format=src&type=gif')
		.then(function(response) {
			var a = i;
			return (function (t) {
					var catImg = document.getElementById('imgBox' + (count+1)).firstChild;
					var catMenuItem = document.getElementById('menuitem'+(count+1));
					console.log('Cat image url: ' + response.url + ', Cat counter: ' + (count+1) + ', ' + catImg);
					catImg.src = response.url;
					catMenuItem.src = response.url;
					count++;
			})(count);
} ));
}
